...
import { Link, Navigate } from "react-router-dom"; // Updated import for Navigate
...
class SignUp extends React.Component {
  ...
  handleInputChange(event) {
    ...
    if (target.type === "password") {
      this.setState((state) => {
        ...
        return {}; // Return an empty object to avoid state update if no change
      });
    }
  }
  ...
  render() {
    ...
    if (this.props.authState === authStates.LOGGED_IN) {
      return <Navigate to="/Home" />; // Updated to use Navigate
    }
